    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        if self.binning:
            ...
            for row in layer_data:
                ...
                # 等距分bin：从最小值到最大值等距划分
                min_val = non_zero_row.min()
                max_val = non_zero_row.max()
            ...
            # 统计并打印各个bin的比例
            all_binned_data = np.stack(binned_rows)
            total_elements = all_binned_data.size
            
            logger.info("Bin distribution statistics:")
            for bin_idx in range(n_bins):
                bin_count = (all_binned_data == bin_idx).sum()
                bin_ratio = bin_count / total_elements * 100
                if bin_idx == 0:
                    logger.info(f"  Bin {bin_idx} (zeros): {bin_count:,} elements ({bin_ratio:.2f}%)")
                else:
                    logger.info(f"  Bin {bin_idx}: {bin_count:,} elements ({bin_ratio:.2f}%)")
            
            adata.layers[self.result_binned_key] = all_binned_data
...
def binning(
    ...
    """
    Equal-width binning (等距分箱) with +1 shift:
    对整行做 min-max 归一化 -> 映射到 [0, n_bins-1] -> floor -> clamp -> +1
    注意：这里没有“零专属 bin”，0 也会被归到某个 [1..n_bins] 的bin中，
    与您给的 discretize_expression 完全一致。
    """
    # 记录类型并统一到 numpy 运算
    is_torch = isinstance(row, torch.Tensor)
    dtype = row.dtype if not is_torch else row.dtype  # 仅为了取回 dtype
    np_row = row.detach().cpu().numpy() if is_torch else row

    # 处理退化情形：整行常数
    rmin = np_row.min()
    rmax = np_row.max()
    if rmax == rmin:
        # 全部值相等 => 归一化全 0，bin 全部为 1（+1 之后）
        out = np.ones_like(np_row, dtype=np.int64)
        return torch.from_numpy(out) if is_torch else out

    # 等距分箱核心
    eps = 1e-8
    norm = (np_row - rmin) / (rmax - rmin + eps)              # [0, 1]
    idx  = np.floor(norm * (n_bins - 1)).astype(np.int64)     # [0, n_bins-1]
