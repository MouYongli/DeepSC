...
class GeneEmbedding(nn.Module):
    ...
    def __init__(self, embedding_dim: int, num_genes: int, padding_idx:int=0):
        super(GeneEmbedding, self).__init__()
        self.embedding_dim = embedding_dim
        self.gene_embedding = nn.Embedding(
            num_embeddings=num_genes + 2, embedding_dim=embedding_dim, padding_idx=padding_idx
        )
...
class ExpressionEmbedding(nn.Module):
    ...
    def __init__(self, embedding_dim: int, num_bins: int = 50, alpha: float = 0.1, padding_idx:int=0):
        """
        Args:
        ...
        self.bin_embedding = nn.Embedding(
            num_embeddings=num_bins + 3, embedding_dim=embedding_dim, padding_idx=padding_idx
        )
        # 连续值的投影向量 v_cont ∈ R^d
...
class DeepSC(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.vocab = vocab
        self.gene_embedding = GeneEmbedding(embedding_dim, num_genes, padding_idx=vocab[pad_token])
        self.expr_embedding = ExpressionEmbedding(
            embedding_dim, num_bins=num_bins, alpha=alpha, padding_idx=pad_value
        )
        self.num_heads = num_heads
        ...
        self.cls_decoder = ClsDecoder(embedding_dim, n_cls, num_layers_cls)
        self.value_encoder = ContinuousValueEncoder(embedding_dim, dropout=0.2)
    ...
    def forward(
        ...
        expr_emb = self.expr_embedding(value_binned, value_log1p)  # (batch, g, d)
        #expr_emb = self.value_encoder(value_log1p)
        ...
        # 对最后一维求和，然后对第3维求平均，得到每个基因的权重
        weights = y.sum(dim=-1).mean(dim=2)  # (batch, g)#用啥得到权重？y还是value？
        #weights = value
