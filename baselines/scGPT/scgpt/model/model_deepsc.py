...
try:
    ...
    logging.info("Warning: Flash Attention not available, falling back to standard attention")
    ...
    def __init__(self, d, hidden_dim=None, dropout=0.1, num_layers=2):
        ...
        if num_layers > 0:
            ...
            for _ in range(num_layers):
                self.layers.append(nn.Sequential(nn.Linear(d, hidden_dim), nn.GELU(), nn.Linear(hidden_dim, d), nn.Dropout(dropout)))
...
class FeedForward(nn.Module):
    def __init__(self, d, hidden_dim=None, dropout=0.1, num_layers=2):
        ...
        # 第一层：d -> hidden_dim
        first_layer = nn.Sequential(nn.Linear(d, hidden_dim), nn.GELU(), nn.Dropout(dropout))
...
class FlashDeepSCTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.use_expr_emb = use_expr_emb
        ...
        # FFN
        if use_new_ffn:
            self.ffn_gene = FeedForwardNew(
                embedding_dim, dropout=ffn_dropout, num_layers=num_layers_ffn, hidden_dim=ffn_dim
            )
            self.ffn_expr = FeedForwardNew(
                embedding_dim, dropout=ffn_dropout, num_layers=num_layers_ffn, hidden_dim=ffn_dim
            )
        else:
    ...
    def forward(self, gene_emb, expr_emb, M=None):
        ...
        if self.use_gene_emb:
            ...
            out_gene = gene_emb
        if self.use_expr_emb:
            # Expression branch
            old_expr_emb = expr_emb
...
class DeepSC(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                FlashDeepSCTransformerBlock(
                    ...
                    num_layers_ffn=ffn_layers,
                    fused=fused,
                    use_gene_emb=self.use_gene_emb,
                    use_expr_emb=self.use_expr_emb,
                    use_new_ffn=use_new_ffn,
                    ffn_dim=ffn_dim,
                )
                for _ in range(num_layers)
            ]
        ...
        encoder_layers = TransformerEncoderLayer(
                embedding_dim, num_heads, dim_feedforward=embedding_dim*4, dropout=ffn_dropout, batch_first=True
            )
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            if i >= self.mask_layer_start and self.calculate_M:
...
class DeepSCClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.cls_decoder = ClsDecoder(deepsc_encoder.gene_embedding.embedding_dim, n_cls, num_layers_cls)
    
    def _get_cell_emb_from_layer(
        self, layer_output, weights = None
    ):
        """
        Args:
