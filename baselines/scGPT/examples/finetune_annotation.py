...
parser = argparse.ArgumentParser(description='Fine-tune scGPT for cell-type annotation')
parser.add_argument('--adata_path', type=str, required=True, help='Path to training adata file')
parser.add_argument('--adata_test_path', type=str, required=True, help='Path to test adata file')
...
# Extract dataset name from the adata_path filename
dataset_name = Path(args.adata_path).stem.replace('_train', '').replace('_test', '')
...
# Use command line arguments for data paths
adata = sc.read(args.adata_path)
adata_test = sc.read(args.adata_test_path)
...
# Compute UMAP coordinates before plotting
# First do PCA if not already done
if 'X_pca' not in adata_test_raw.obsm.keys():
    sc.tl.pca(adata_test_raw, svd_solver='arpack')
...
# 获取每个类别的详细指标
target_names = [id2type[i] for i in sorted(set(labels) | set(predictions))]
...
fig.suptitle('Classification Metrics by Cell Type', fontsize=16, fontweight='bold')
...
ax4.set_ylim(0, max(proportions) * 1.1)
...
# 记录到wandb
results["test/classification_metrics"] = wandb.Image(
    str(save_dir / "classification_metrics.png"),
    caption=f"Classification metrics by cell type",
)
...
metrics_df.to_csv(save_dir / "classification_metrics.csv", index=False)
print("\nClassification Metrics by Cell Type:")
