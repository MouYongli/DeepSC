...
from deepsc.data.dataset import SCDataset, extract_rows_from_sparse_tensor, data_mask
...
class Trainer:
    ...
    def load_data(self):
        ...
        self.train_dataset = SCDataset(data_train, self.args.bin_num)
        self.val_dataset = SCDataset(data_val, self.args.bin_num)
    ...
    def prepare_model(self):
        ...
        self.model, self.optimizer = self.fabric.setup(args.model, self.optimizer)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epoch + 1):
            ...
            for index, data in enumerate(self.train_loader, start=1):
                index += 1
                logging.info(f"Accumulated gradient on rank:{self.fabric.global_rank} and in step {index}")
            ...
            if epoch % self.args.valid_every == 0:
                self.validate(epoch)
            if self.is_master:
                save_ckpt(epoch, self.model, self.optimizer, self.scheduler, epoch_loss, self.args.model_name, self.args.ckpt_dir)
