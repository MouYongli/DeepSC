...
class SCDataset(Dataset):
    def __init__(self, coo_tensor, bin_num=5):
    ...
    def __getitem__(self, idx):
        ...
        row[row > self.bin_num] =self.bin_num # 需要更改
        row = row.long()
...
def normalize_tensor(csr):
    ...
    row_scales = np.divide(1e4, row_sums, out=np.zeros_like(row_sums, dtype=np.float64), where=row_sums > 0)
...
def preprocess_sparse_tensor(pth_data_path, save_path=None):
    ...
    if not coo_tensor.is_coalesced():
        raise ValueError("The input sparse tensor is not coalesced. Please call coalesce() first.")
