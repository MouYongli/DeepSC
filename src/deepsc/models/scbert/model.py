...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v, output_attentions=False):
        device = q.device
        # inds = [8060, 8064, 6243, 8575, 10342, 10913, 9366, 993, 7796, 5210,
        #  5212, 5504, 6851, 6559, 5508, 13107, 13820]
        ...
        if output_attentions:
            ...
            for head_dim in range(q.shape[1]):
                # attn_weights[0, head_dim] = torch.abs(attn_fn(q[:,head_dim].to(torch.float16),
                # k[:,head_dim].to(torch.float16), v_diag[:,head_dim].to(torch.float16)))[0, inds][:, inds]
                ...
                # attn_weights += norm_tensor(torch.abs(attn_fn(q[:,head_dim].to(torch.float16),
                # k[:,head_dim].to(torch.float16), v_diag[:,head_dim].to(torch.float16))), dim=-1)
...
class Performer(nn.Module):
    def __init__(
        ...
        # , where d is the dimension of each head ?? what is random feature ??
        ...
        generalized_attention=False,  # defaults to softmax approximation, but can be set to True for
        # generalized attention ?? what is generalized attention ??
        kernel_fn=nn.ReLU(),  # the kernel function to be used, if generalized attention is turned on, defaults to Relu
        use_scalenorm=False,  # use scale norm, from 'Transformers without Tears' paper, a
        # substitute for LayerNorm, priority: scalenorm.rezero.layernorm
        use_rezero=False,  # use Rezero or not, from 'Rezero is all you need'
        # paper, a substitute for LayerNorm, priority: scalenorm.rezero.layernorm
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        cross_attend=False,
        no_projection=False,
        tie_embed=False,  # False: output is num of tokens, True: output is dim of tokens
